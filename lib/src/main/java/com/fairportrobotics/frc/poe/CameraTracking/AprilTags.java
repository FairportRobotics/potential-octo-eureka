...
public class AprilTags { 
    ...
    public List<PhotonTrackedTarget> getTargets() {
        ...
        for(PhotonCamera camera : cameras) {
            for(PhotonTrackedTarget target : camera.getLatestResult().getTargets()) {
                ...
                if(toAdd) targets.add(target);
            }
            targets.addAll(camera.getLatestResult().getTargets());
        }
    ...
    public boolean hasTargets() {
        for(PhotonCamera camera : cameras) {
            // if a camera has targets, return true
            if(camera.getLatestResult().hasTargets()) return true;
        } 
